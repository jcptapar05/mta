...
const ShowroomNavbar = () => {
  ...
  return (
    <nav
      className="flex flex-row align-center items-center justify-between px-4 md:px-0 md:justify-around bg-white bg-opacity-50"
      style={{
        // position: "sticky",
        // top: 0,
        zIndex: 50,
      ...
      <ul className="md:flex items-center flex-row align-center h-full hidden">
        {sidebarItems.map((item) => (
          ...
          >
            {item.link && (
              <Link
                className={`px-3 h-[70px] hover:bg-gray-200 text-[14px] flex items-center ${
